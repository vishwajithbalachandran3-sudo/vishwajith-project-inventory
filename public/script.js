...
function renderMaintenance(){const today=new Date();today.setHours(12,0,0,0);const data=[[3,'Packaging Line 02','PR-4838 · 4 materials required','Ready','ready'],[7,'Assembly Line 07','PR-4841 · 6 materials required','1 material at risk','risk'],[12,'Finishing Line 03','PR-4846 · 3 materials required','Ready','ready']].map(x=>{const date=new Date(today);date.setDate(date.getDate()+x[0]);return[date,...x.slice(1)]});const month=d=>d.toLocaleString('en-US',{month:'short'}).toUpperCase();const first=data[0][0],last=data[data.length-1][0];const heading=document.querySelector('#maintenanceMonth');if(heading)heading.textContent=first.getMonth()===last.getMonth()?`${month(first)} ${first.getFullYear()}`:`${month(first)}–${month(last)} ${last.getFullYear()}`;document.querySelector('#maintenanceList').innerHTML=data.map(x=>`<div class="maintenance-item"><div class="date-block"><small>${month(x[0])}</small><strong>${x[0].getDate()}</strong></div><div class="maintenance-copy"><strong>${x[1]}</strong><small>${x[2]}</small></div><span class="readiness ${x[4]}">${x[3]}</span></div>`).join('')}
...
(function(){
  ...
  function showDialog(title,html){let d=document.querySelector('#quickDialog');if(!d){d=document.createElement('div');d.id='quickDialog';d.className='modal-backdrop';d.innerHTML='<div class="modal"><button class="modal-close">×</button><p class="eyebrow">INVENTORY OPERATIONS</p><h2></h2><div class="dialog-body"></div><div class="modal-actions"><button class="primary dialog-done">Done</button></div></div>';document.body.appendChild(d);d.addEventListener('click',e=>{if(e.target===d||e.target.closest('.modal-close')||e.target.closest('.dialog-done'))d.classList.remove('open')})}d.querySelector('h2').textContent=title;d.querySelector('.dialog-body').innerHTML=html;d.classList.add('open')}
  document.addEventListener('click',function(e){
    ...
    const action=e.target.closest('.stock-action');if(action){const p=parts[+action.dataset.i],receiving=action.dataset.type==='receive',answer=prompt(`${receiving?'Receive':'Issue'} how many units of ${p.name}?`,'1');const qty=Number(answer);if(Number.isFinite(qty)&&qty>0){if(!receiving&&qty>p.stock){showToast('Not enough stock',`Only ${p.stock} units are currently available.`);return}p.stock+=receiving?qty:-qty;persistStockLevels();renderParts(document.querySelector('#partSearch').value,document.querySelector('#statusFilter').value);renderAttention();document.querySelector('#quickDialog').classList.remove('open');showToast('Inventory updated',`${p.name} now has ${p.stock} units available and is ${statusText(status(p)).toLowerCase()}.`)}return}
  ...
  const help=document.querySelector('.help');if(help)help.onclick=()=>showDialog('Inventory quick guide','<p><strong>Find a product:</strong> use the search bar or Product inventory page.</p><p><strong>Move stock:</strong> click the ••• button beside a product, then choose Issue or Receive.</p><p><strong>Reorder:</strong> click New purchase order and submit the form.</p>');
  const notify=document.querySelector('.icon-btn');if(notify)notify.onclick=()=>{const expected=orders.find(order=>order[0]==='PO-2048')?.[3]||formatOrderDate(4);showDialog('Notifications',`<p>🔴 8 products are critically low.</p><p>📦 PO-2048 is expected on ${expected}.</p><p>⚠ Siemens Proximity Sensor is unavailable for the upcoming production schedule.</p>`)};
  const profile=document.querySelector('.profile');if(profile)profile.onclick=()=>showDialog('Vishwajith','<p>Vish Plant</p><p style="color:#6c756f">Permissions: inventory, procurement, reports</p>');
